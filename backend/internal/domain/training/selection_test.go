...
func TestSelectProblemSetUsesDistinctContestsAndConfiguredOrder(t *testing.T) {
	...
	if level != 0 || len(selected) != 5 { t.Fatalf("level=%d len=%d", level, len(selected)) }
	used := map[string]bool{}
	wantSlots := []string{"D1","E1","E2","E3","F1"}
	for index, item := range selected {
		if item.Slot.Name != wantSlots[index] { t.Fatalf("slot %d = %s", index, item.Slot.Name) }
		if used[item.Problem.ContestID] { t.Fatalf("duplicate contest %s", item.Problem.ContestID) }
